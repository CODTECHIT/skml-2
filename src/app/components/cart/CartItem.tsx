...
export function CartItem({ item, product }: { item: { id: number; quantity: number }; product: Product }) {
  ...
  return (
    <div className="flex gap-4 p-4 bg-card rounded-2xl border border-border shadow-sm">
      ...
      <div className="flex-1 flex flex-col justify-between">
        <div>
          <h3 className="font-poppins font-semibold text-foreground text-sm line-clamp-2">{product.name}</h3>
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex items-center gap-3 bg-muted px-2 py-1 rounded-full border border-border">
            <button onClick={() => decreaseQuantity(item.id)} className="p-1 hover:text-primary transition-colors">
              <Minus size={14} />
            </button>
            <span className="text-sm font-semibold w-4 text-center">{item.quantity}</span>
            <button onClick={() => increaseQuantity(item.id)} className="p-1 hover:text-primary transition-colors">
              <Plus size={14} />
            </button>
          </div>
